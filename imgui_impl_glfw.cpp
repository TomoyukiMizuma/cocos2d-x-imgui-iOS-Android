...
bool ImGui_ImplGlfw_Init(bool install_callbacks)
{
    ...
    cocos2d::GLViewImpl* glViewImpl = static_cast<cocos2d::GLViewImpl*>(glView);
    g_Window = glViewImpl->getWindow();
    g_Time = 0.0;
...
#else // (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32) || (CC_TARGET_PLATFORM == CC_PLATFORM_MAC)

    io.BackendPlatformName = "imgui_impl_gles";
//    io.ConfigFlags |= ImGuiConfigFlags_IsTouchScreen;
    auto listener = cocos2d::EventListenerTouchOneByOne::create();
    listener->onTouchBegan = [](cocos2d::Touch* touch, cocos2d::Event* event) -> bool
    { // g_MouseJustPressed[0]...LMB, [1]...RMB, [2]...MMB
        auto glview = cocos2d::Director::getInstance()->getOpenGLView();
        const auto frame_size = glview->getFrameSize();
        const auto reso_size = glview->getDesignResolutionSize();
        cocos2d::Vec2 point = touch->getLocationInView();
        g_CursorPos.x = point.x * frame_size.width / reso_size.width;
        g_CursorPos.y = point.y * frame_size.height / reso_size.height;
...
#else // (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32) || (CC_TARGET_PLATFORM == CC_PLATFORM_MAC)
    io.MouseDown[0] = g_MouseJustPressed[0];
    g_MouseJustPressed[0] = false;
    io.MousePos = ImVec2(-FLT_MAX, -FLT_MAX);
    if (io.WantSetMousePos)
    {
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    w = frame_size.width;
    h = frame_size.height;
    display_w = frame_size.width * glview->getRetinaFactor();
    display_h = frame_size.height * glview->getRetinaFactor();
#endif
    ...
    g_Time = current_time;
#else // (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32) || (CC_TARGET_PLATFORM == CC_PLATFORM_MAC)
    float deltaTime = cocos2d::Director::getInstance()->getDeltaTime();
